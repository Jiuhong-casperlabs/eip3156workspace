...
use constants::{
    ...
    FEE_KEY_NAME, NAME_KEY_NAME, SYMBOL_KEY_NAME, TOTAL_SUPPLY_KEY_NAME,
};
pub use error::Error;
...
impl ERC20 {
    ...
    pub fn loan_fee(&self) -> U256 {
        detail::read_from(FEE_KEY_NAME)
    }
    // EIP3156 end
    ...
    // EIP3156 start
    /// Returns the max flash loan of the token.
    pub fn max_flash_loan(&self) -> U256 {
        U256::MAX - self.read_total_supply()
    }

    /// Returns flash fee for the token
    pub fn flash_fee(&mut self, token: Address, amount: U256) -> Result<U256, flashmintError> {
        // require(
        //     token == address(this),
        //     "FlashMinter: Unsupported currency"
        // );

        flashmint::flash_fee(self, token, amount)
    ...
    pub fn install_custom(
        ...
        // EIP3156 start
        let fee_key = {
            let fee_uref = storage::new_uref(fee).into_read();
            Key::from(fee_uref)
        };
        // EIP3156 end
